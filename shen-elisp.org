...
In the case of [[(dotted-pairs)][dotted pairs]] which are sometimes generated from KLambda we
explicitly test the first and the second element. For [[(normal-lists)][all other lists]] iterate
over the elements checking each one against the element from the corresponding
list and in the case of [[(hash-tables)][hash tables]] check that the keys and values match.
...
When bootstrapping from the seed KLambda files we first need to [[Generate From Seed KLambda][patch]] the
